...
HelpInfo = 'python SCOUT.py -N <ProjectName> -r <reference> -i <inputBam> -o <outputdir> -c <chromID> -S <startPos> -E <EndPos> -W <WindowSize, default=30000> -M <decay mu value, default=ln(3)/10> -T <Parameter Table> -P <Number of CPU>'
...
def main(argv):
    ...
    try:
        opts,args = getopt.getopt(argv, "hN:r:i:o:c:S:E:W:M:P:T:", ['Name=', 'ref=', 'ifile=', 'ofile=', 'chr=', 'Start=', 'End=', 'WindowSize=', 'Mu=', "Process=", "Table="])
    ...
    for opt, arg in opts:
        ...
        elif opt in ("-T", '--Table'):       # parameter table
            TableFile = arg
    if TableFile:
        try:
            f = open(TableFile)
        except:
            print('Parameter Table do not exist, please check parameter table format in <NBJUDGE_home>/example')
            print(HelpInfo)
            sys.exit(2)
        print('Detect the TableFile parameter, adjust all parameters according to the TableFile')
    ...
    if not os.path.exists(resultDir):
        os.mkdir(os.path.join(Parameter['outputdir'], Parameter['PName']))
    ResultPool = {}
    ...
    Candidate.to_csv("%s.CandidateDf.csv" % PName)
    ABdf.to_csv('%s.ABdf.csv' % PName)
    ...
    with open('%s.Parameter.OutPut.txt' % PName, 'w') as output:
        output.write('ProjectName:%s\nreference:%s\nsamfile:%s\nchrom:%s\nStart:%s\nEnd:%s\nWindowSize:%s\nmu:%s\n' % (PName, Parameter['refFile'], Parameter['samfile'], Parameter['chrom'], Parameter['Start'], Parameter['End'], Parameter['WindowSize'], Parameter['mu']))
    print('%s: %s Finished' % (time.ctime(), PName))
